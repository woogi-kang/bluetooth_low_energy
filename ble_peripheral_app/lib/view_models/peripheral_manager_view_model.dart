...
class PeripheralManagerViewModel extends ViewModel {
  ...
  bool get hasAuthenticatedCentrals => _authenticatedCentrals.values.any((authenticated) => authenticated);
  ...
  void _setupSubscriptions() {
    ...
    _characteristicNotifyStateChangedSubscription = _manager.characteristicNotifyStateChanged.listen((eventArgs) {
  ...
  void _initializeDevice() {
    if (Platform.isAndroid || Platform.isIOS) {
      _deviceInfo = Platform.isAndroid ? 'Android BLE Peripheral' : 'iOS BLE Peripheral';
    } else {
  ...
  Future<void> startAdvertising() async {
    ...
    try {
      // 서비스 및 특성 정의
      final serviceUuid = UUID.fromString('12345678-1234-5678-9abc-def012345678');
      final readCharacteristicUuid = UUID.fromString('87654321-4321-8765-cbad-fed098765432');
      final writeCharacteristicUuid = UUID.fromString('11111111-2222-3333-4444-555555555555');
      final notifyCharacteristicUuid = UUID.fromString('aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee');
  ...
  void _handleCharacteristicRead(eventArgs) async {
    ...
    if (!_authenticatedCentrals.containsKey(central) || !_authenticatedCentrals[central]!) {
      ...
      final deviceData = 'DEVICE_INFO:$_deviceInfo|TIMESTAMP:${DateTime.now().millisecondsSinceEpoch}';
      await _manager.respondReadRequestWithValue(
        request,
  ...
  void _handleNotifyStateChanged(eventArgs) {
    ...
    _addLog(LogEntry(
      level: LogLevel.info,
      message: 'Notify 상태 변경: Central ${central.uuid} - ${enabled ? '활성화' : '비활성화'}',
      timestamp: DateTime.now(),
    ));
  ...
  Future<void> startVoiceRecording() async {
    ...
    try {
      ...
      await _audioRecorder.start(const RecordConfig(), path: '/tmp/voice_message.m4a');
      _isRecording = true;
